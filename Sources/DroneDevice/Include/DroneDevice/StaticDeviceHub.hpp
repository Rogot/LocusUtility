...
private:
	...
	struct DeviceIterator {
		...
		{
			aToken->refs = static_cast<decltype(aToken->refs)>(sizeof...(Ts) - N - 1);

			if ((aDevice == kDeviceReservedId || aDevice == std::get<N>(aTuple).address())
				&& std::get<N>(aTuple).address() != kDeviceReservedId) {
				if (aToken != nullptr) {

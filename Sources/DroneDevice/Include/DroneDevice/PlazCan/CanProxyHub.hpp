...
template<typename Component, size_t maxNodeCount, size_t maxRequestCount = 8, size_t maxFieldCount = 64,
	unsigned int nodeAllocationDelay = 0>
class CanProxyHub : public Component {
	using microseconds = std::chrono::microseconds;
	using milliseconds = std::chrono::milliseconds;

	using Component::handler;
	using CurrentType = CanProxyHub<Component, maxNodeCount, maxRequestCount, maxFieldCount, nodeAllocationDelay>;
		...
		if (Component::currentAddress != kAddressUnallocated
			...
			switch (static_cast<DataType::Message>(aTransfer->data_type_id)) {
				...
				case DataType::Message::ALLOCATION:
					if (allocation.enabled && TimeType::milliseconds() > milliseconds{nodeAllocationDelay}) {
...
template<typename Component, size_t maxNodeCount, size_t maxRequestCount, size_t maxFieldCount,
	unsigned int nodeAllocationDelay>
constexpr std::chrono::milliseconds
	CanProxyHub<Component, maxNodeCount, maxRequestCount, maxFieldCount, nodeAllocationDelay>::kOfflineTimeout;

template<typename Component, size_t maxNodeCount, size_t maxRequestCount, size_t maxFieldCount,
	unsigned int nodeAllocationDelay>
constexpr std::chrono::milliseconds
	CanProxyHub<Component, maxNodeCount, maxRequestCount, maxFieldCount, nodeAllocationDelay>::kRequestTimeout;

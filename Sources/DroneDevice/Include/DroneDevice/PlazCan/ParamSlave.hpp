			...
			case DataType::Service::PARAM_GET_SET: {
				...
				if (request) {
					...
					if (nameLength) {
						...
						fieldRequestIndex(nameBuffer, request);
					} else {
						// Read or write field
...
protected:
	...
	Device::FieldId getFieldIndex(const char *aName)
	{
		for (Device::FieldId i = 0; i < parent.device.getFieldCount(); ++i) {
			if (strcmp(parent.device.getField(i)->name(), aName) == 0) {
				return i;
			}
		}

		return Device::kFieldReservedId;
	}
...
private:
	void fieldRequestIndex(const char *aField, Device::RefCounter *aToken)
	{
		const auto index = getFieldIndex(aField);

		if (index != Device::kFieldReservedId) {
			onFieldIndexReceived(index, aToken);
		} else {
			onFieldIndexRequestError(Device::Result::FIELD_NOT_FOUND, aToken);
		}
	}

	void onFieldIndexReceived(Device::FieldId aField, Device::RefCounter *aToken)
	{
		parent.device.fieldRequestInfo(aField, this, aToken);
	}

	void onFieldIndexRequestError(Device::Result /*aError*/, Device::RefCounter *aToken)
	{
		const auto * const request = static_cast<const ParamRequestDescriptor *>(aToken);
		sendEmptyGetSetResponse(request);
		pool.free(request);

...
public:
	...
	{
		return size() > 0 && (readable || !Interface::isProtected());
	}

	static bool isWritable()
	{
		return size() > 0 && (writable || !Interface::isProtected());
	}
	...
	{
		assert(position + length <= size() && "Attempt to write outside the region");
